...
from task1 import can_satisfy_demand as can_meet_demand
...
def test_can_meet_demand(costs, resource_amounts, demand_expected, expected):
    if isinstance(expected, type) and issubclass(expected, Exception):
        with pytest.raises(expected):
            can_meet_demand(costs, resource_amounts, demand_expected)
    else:
        assert can_meet_demand(costs, resource_amounts, demand_expected) == expected
