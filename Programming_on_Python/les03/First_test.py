import pytest
from first.first import ShapeMismatchError
from first.first import sum_arrays_vectorized, compute_poly_vectorized

from first.first import get_mutual_l2_distances_vectorized, get_mutual_l2_distances_naive
import numpy as np
...
def test_compute_poly_standard():
    ...
    expected = np.array([1, 6, 17, 34])
    assert np.array_equal(result, expected)

def test_compute_poly_negative_values():
    x = np.array([-1, -2, -3])
    result = compute_poly_vectorized(x)
    expected = np.array([2, 9, 22])  # y(-1) = 2, y(-2) = 9, y(-3) = 22
    assert np.array_equal(result, expected)

def test_compute_poly_large_values():
    x = np.array([100, 200, 300])
    result = compute_poly_vectorized(x)
    expected = np.array([30201, 120401, 270601])
    assert np.array_equal(result, expected)
